...
flowchart TB
  subgraph Endpoints["Monitored endpoints"]
    Win["Windows Server 2022"]
    Future["Future: firewall & homelab hosts"]
  end

  subgraph WazuhVM["Wazuh VM · Proxmox · 4c / 8GB"]
    Agent["Wazuh agents"]
...
Creating a new empty virtual machine and allocated enough resources : Wazuh recommendation (4 cores & 8GB RAM)
...
You eventually want to change the default admin password. In order to do this, you will need to use the Wazuh password bash-script, which can be retrieved from their site to the server:
...
## Bonus - Postgres Container Test

I assigned only 1 core and 1 GB ram for my Postgres DB-instance. Wanted to stresstest it and created this loop, which will import data into *categories* -table.
````
do
$$
begin
   for counter in 1..1000000 loop
	insert into categories (category_name, description) 
       values(counter,counter); -- Uses the counter as input value so it will be different, since increments in each loop.
   end loop;
end;
$$;
````

Seems to be pretty unstable, eventually the usage was in 99%. Just a test for gathering live logs from my firewall or endpoints in the future.
